...
### 座標系の詳細 (MediaPipe Face Landmarker)

**変換行列からの位置抽出**:
- MediaPipeの `facialTransformationMatrixes` は列優先（column-major）形式の4x4行列
- 位置成分は行列の最終列: `matrix[12]`, `matrix[13]`, `matrix[14]`

**座標軸の定義** (実機観察結果):
- **X軸 (px)**: ユーザーが左に移動→正の値、右に移動→負の値
- **Y軸 (py)**: ユーザーが立ち上がる→正の値、かがむ→負の値
- **Z軸 (pz)**: カメラから離れる→負の値（絶対値大）、近づく→負の値（絶対値小）
  - カメラまでの距離に負符号をつけた値として解釈可能

**回転 (Quaternion)**:
- 変換行列の回転成分（3x3部分）から四元数に変換
- 変換アルゴリズムは標準的な行列→四元数変換を使用
