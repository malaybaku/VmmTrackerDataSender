...
1. 静的なwebページであって、MediaPipeを利用した顔トラッキングを行い、トラッキング結果(頭部姿勢、ブレンドシェイプ)を、WebSocketを用いてローカルエリアネットワーク内のPCに送信するようなもの。特に、モバイルで動作することを想定したもの。
...
- ~~DataChannelを今回のような目的で使うことの妥当性~~ → **WebSocketを採用** (後述)
...
### 通信プロトコル: WebSocket採用 (WebRTCは不採用)

**決定日**: 2026-02-15

**採用技術**: WebSocket

**不採用技術**: WebRTC DataChannel

**理由**:
LAN内通信におけるWebRTC DataChannelとWebSocketの性能比較調査を実施した結果、以下の結論を得た:

1. **レイテンシ性能差がほぼない**
   - LAN内でのUDP/TCP差は4μs程度(0.004ms)と極めて微小
   - WebRTC DataChannelとWebSocketのRTTは両者とも20-40ms程度
   - 顔トラッキング処理自体が50-100msかかることを考慮すると、ネットワーク伝送の差(1-4ms)は誤差範囲(全体の1-4%)

2. **LAN環境の特性**
   - パケットロスがほぼゼロのため、TCPの再送オーバーヘッドが問題にならない
   - 低レイテンシ環境のため、TCPの輻輳制御による遅延も最小化される
   - UDPの「確認応答不要」による速度優位性が実質的に無効化される

3. **実装の複雑さ**
   - WebRTCはシグナリング(SDP交換)が必須であり、静的ページからの実装が複雑
   - WebSocketは単純なクライアント-サーバー接続で実装が容易
   - 今回のユースケースは一方向データ送信であり、P2Pのメリットが活かせない

4. **保守性・デバッグ性**
   - WebSocketの方がデバッグツールが充実
   - .NETでの実装がシンプル(System.Net.WebSockets標準サポート)

**結論**: 実装の複雑さに見合う性能メリットが得られないため、WebSocketを採用する。
